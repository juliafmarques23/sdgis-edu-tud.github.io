...
- Confirm the AHN DTM is pre-clipped and delivered via the same starter-project mechanism as Part 1 (per the workshop plan, raw AHN pulls are "slow/unpredictable live", so students should not be fetching it themselves) –\> **done**
- Confirm whether Part 2 opens its own starter project (`[project-name-part2].qgz` below) or continues in the same project file as Part 1 — written here as a separate project, adjust if it should chain from Part 1's output –\> **done**
- Pick the actual cpt-city colour ramp for Step 3 (left as a placeholder — QGIS ships several elevation/terrain ramps under Color ramp → All Color Ramps → cpt-city) –\> **we decided to let students choose the exact color ramp from the catalog**
- Decide the output naming/format for the exported risk-zone raster in Step 5 — flagged as open in the original draft –\> **done**
- Confirm the 0.70 m threshold and its World Bank Climate Knowledge Portal source before this goes to students –\> **could not find this exact information** (still looking for it or an alternative source)
...
- Fill in the real project filename, AHN DTM filename, cpt-city ramp name, and risk-zone output name/format (currently `[bracketed]` placeholders). –\> **done**
- Confirm the 0.70 m 2100 sea-level-rise figure and its World Bank Climate Knowledge Portal citation before publishing — it's used here as a simple teaching threshold, not a validated flood-risk figure, and Step 3's reflection question is designed to surface that limitation with students. –\> **still checking source**
- Consider whether the histogram/min-max questions (Step 2.2) need a fixed expected answer once the real AHN extract is finalised, rather than the open-ended guidance currently given. –\> **I think guidance enough**
