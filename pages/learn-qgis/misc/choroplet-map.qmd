...
- Confirm the real starter project filename and the exact field names in `PV29__00_Basis_Bouwblok` (the draft this was built from used `GM_CODE`, `FSI_2024`/`FSI_24`, and `FSI_19` inconsistently — pick one naming convention and confirm it against the actual dataset) –\> **done**
- Confirm the Vlissingen municipality code (`GM0718`) is correct and correctly quoted as a string in the expression –\> **done**
- **Unlike Parts 1 and 2, the draft has students filter Vlissingen out of a province-wide (Zeeland) layer themselves, rather than starting from data pre-clipped to the study area.** Decide whether that's a deliberate teaching point for this block (practising `Select by Expression`) or should be pre-clipped like the other parts to save time — Step 1 below is written assuming it's deliberate –\> **done**
- Confirm the RUDIFUN background text below (translated/condensed from the original Dutch source blurb) is accurate before publishing –\> **from what I could find, yes**
...
- Fill in the real project filename, output layer names, and the exact FSI field names (currently `[bracketed]` placeholders) once confirmed against the actual dataset. –\> **done**
- Confirm the `GM0718` municipality code for Vlissingen and the exact FSI field names before this goes to students — the original draft used `FSI_2024` and `FSI_24` interchangeably. –\> **done**
- Decide whether students genuinely select Vlissingen out of the full Zeeland layer (Step 1, as written) or start from data already clipped to the study area like Parts 1 and 2 — this materially affects timing in a 45-minute slot. –\>**done** (we just make sure the path to the shapefile doesn't get lost)
